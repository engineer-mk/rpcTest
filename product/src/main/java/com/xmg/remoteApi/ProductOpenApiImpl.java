...
@RpcProvider
@Service
public class ProductOpenApiImpl implements ProductOpenApi {
    @Autowired
    private ProductRepository productRepository;
    @Autowired
    private AccountApi accountRemoteApi;
    @Autowired
    private EntityManager entityManager;
    @Autowired
    @Lazy
    private ProductOpenApiImpl productApi;

    public void reduceCount(Long id, Long count) {
        productApi.doReduceCount(id, count);
        accountRemoteApi.deductBalance(1L, 10d * count);
    }
